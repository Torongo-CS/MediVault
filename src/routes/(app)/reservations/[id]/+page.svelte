...
<script lang="ts">
  ...
  const resId = $derived(page.params.id as Id<"reservations">);

  let rawReservation = $state<any>(null);
  ...
  $effect(() => {
    ...
    unsubscribe = convex.onUpdate(
      api.reservations.getByIdWithDetails,
      { reservationId: resId },
      (data) => {
        rawReservation = data;
      }
  ...
  const reservation = $derived({
    id: resId,
    pharmacyName: rawReservation?.pharmacistName || "HealthPlus Pharmacy",
    pharmacyAddress: "42 Mirpur Road, Dhaka 1205", // Mock address
    pharmacyPhone: "+880 1712-345678",
    status: (rawReservation?.status || "pending") as
      | "pending"
      | "approved"
      | "rejected"
      | "expired"
      ...
      | "delivered"
      | "cancelled",
    requestDate: rawReservation?.createdAt || Date.now(),
    pickupDate: rawReservation?.pickupDate || (Date.now() + 12 * 60 * 60 * 1000),
    totalAmount: rawReservation?.totalCosting || 0,
    invoiceNumber: rawReservation?._id ? `INV-${rawReservation._id.slice(-6).toUpperCase()}` : "INV-XXXXXX",
    pharmacistNote: "All items verified. Please bring your NID card for pickup.",
    interactionWarnings: [] as string[],
    items: rawReservation?.medsList?.map((item: any) => ({
      name: item.name,
...
<div class="space-y-6 max-w-4xl mx-auto">
  ...
  >
    <div>
      <h1 class="text-xl font-bold text-foreground">
        Reservation #{reservation.invoiceNumber}
      </h1>
      <p class="text-xs text-muted-foreground mt-0.5">
        Placed {formatDate(reservation.requestDate)}
      </p>
    </div>
  ...
  {#if !isTerminal}
    <Card.Root class="p-5">
      <h3
        class="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4"
      >
        Order Progress
      </h3>
      <div class="flex items-center justify-between relative">
        ...
        <div
          ...
          style="width: {currentIdx >= 0
            ? (currentIdx / (steps.length - 1)) * (100 - 16)
            : 0}%"
        ></div>
  ...
  {:else}
    <Card.Root class="p-4 border-destructive/30 bg-destructive/5">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-semibold text-destructive capitalize"
  ...
  <Card.Root class="p-4">
    ...
    <div class="flex items-start gap-3">
      ...
      <div>
        ...
        <div
          class="flex flex-wrap gap-x-4 gap-y-1 mt-1 text-xs text-muted-foreground"
        >
          <span class="flex items-center gap-1"
            ><MapPin class="h-3 w-3" />{reservation.pharmacyAddress}</span
          >
          <span class="flex items-center gap-1"
            ><Phone class="h-3 w-3" />{reservation.pharmacyPhone}</span
          >
        </div>
      </div>
    </div>
  ...
  <Card.Root class="overflow-hidden">
          ...
          <Table.Row>
            <Table.Cell>
              <div>
                ...
                {#if item.rx}
                  <span
                    class="inline-flex items-center gap-0.5 text-[10px] text-amber-600 dark:text-amber-400 font-medium mt-0.5"
                  >
            ...
            <Table.Cell class="text-right text-sm"
              >৳{item.unitPrice.toFixed(2)}</Table.Cell
            >
            <Table.Cell class="text-right text-sm font-semibold"
              >৳{item.subtotal.toFixed(2)}</Table.Cell
            >
          </Table.Row>
        {/each}
      </Table.Body>
    ...
    <div class="p-4 border-t flex justify-between items-center">
      ...
      <span class="text-xl font-bold text-foreground"
        >৳{reservation.totalAmount.toFixed(2)}</span
      >
    </div>
  </Card.Root>
  ...
  {#if reservation.pharmacistNote}
    <Card.Root class="p-4">
      <h3
        class="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5"
      >
        <MessageSquare class="h-3.5 w-3.5" /> Pharmacist Note
      </h3>
      <blockquote
        class="border-l-2 border-primary pl-3 text-sm text-muted-foreground italic"
      >
